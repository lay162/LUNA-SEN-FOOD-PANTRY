/**
 * Mirror of src/utils/formEmailPayload.js (CommonJS for Cloud Functions).
 * When changing referral email layout, update both files.
 */
...
function buildReferralSummaryLines(data) {
  ...
  if (Array.isArray(d.supportType) && d.supportType.length)
    notesBits.push(`Support requested: ${d.supportType.join(", ")}`);
  if (d.householdItemsNotes) notesBits.push(`Household items notes: ${d.householdItemsNotes}`);
  if (d.applyingFor === "on_behalf") {
    notesBits.push(
      `On behalf of another household — org: ${d.submitterOrganisation || "—"}, role: ${d.submitterCapacity || "—"}${
        d.submitterCapacityDetail ? ` (${d.submitterCapacityDetail})` : ""
      }`
    );
  }
...
function formatFormBodyPlain(formType, data) {
  const lines = [];
...
function buildReferralHtml(data) {
  ...
  if (Array.isArray(d.supportType) && d.supportType.length)
    noteParts.push(`Support requested: ${d.supportType.join(", ")}`);
  if (d.householdItemsNotes) noteParts.push(`Household items: ${d.householdItemsNotes}`);
  if (d.applyingFor === "on_behalf") {
    noteParts.push(
      `Referral on behalf of another household. Organisation: ${d.submitterOrganisation || "—"}. Role: ${
        d.submitterCapacity || "—"
      }${d.submitterCapacityDetail ? `. Detail: ${d.submitterCapacityDetail}` : ""}`
    );
  }
  ...
  const head = `<p style="margin:0 0 16px;font-size:13px;color:#666;">LUNA SEN PANTRY · Wirral · <time>${escapeHtml(
...
function buildGenericHtml(formType, data) {
  ...
  return `<div style="font-family:Segoe UI,Roboto,Helvetica,Arial,sans-serif;max-width:640px;margin:0 auto;">${inner}<p style="font-size:12px;color:#888;margin-top:16px;">${escapeHtml(
    new Date().toISOString()
  )}</p></div>`;
}

function buildFormEmailHtml(formType, data) {
  return formType === "referral" ? buildReferralHtml(data || {}) : buildGenericHtml(formType, data || {});
}

function buildFormEmailSubject(formType) {
...
function formReplyToEmail(data) {
